...
import { useModes, primeModesCache } from '@/hooks/useModes';
...
export default function DisplayClient({
  ...
  initialDisplayState: DisplayState | null;
  initialModes: Mode[];
}) {
  ...
  const displayState = useDisplayStateRealtime(initialLocked ? null : initialDisplayState);
  const activeMode = useActiveMode();
  primeModesCache(initialModes);
  ...
  // Drives the toast's CSS fade — keeps the last message rendered during the
  // 300ms fade-out instead of unmounting the instant `toast` clears.
  const [renderedToast, setRenderedToast] = useState<string | null>(null);
  ...
  useDisplayDeviceHeartbeat(modeId, !locked);
  ...
  return (
    ...
    >
      ...
      {/* Cinema mode toast — plain CSS transition instead of framer-motion,
          which otherwise gets pulled into the bundle for this one toast. */}
      {renderedToast && (
        <div
        ...
        >
          ...
          >
            {cinema ? '⬛  Cinema Mode  —  Double-click or F to exit' : renderedToast}
          </div>
        </div>
      )}
    </div>
